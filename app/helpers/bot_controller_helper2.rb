module BotControllerHelper2
  ...
  def get_tv(show)
    ...
    if $episode[0].finale_aired < Time.now.strftime("%Y-%m-%d") && $episode[0].status == "Ended"
      "#{$episode[0].series_name}: #{$episode[0].overview}. The show ended on #{$episode[0].finale_aired}"
    else
      "#{$episode[0].series_name}: #{$episode[0].overview}. The show airs on #{$episode[0].airs_day_of_week} at #{$episode[0].airs_time}"
    end
  end
  ...
  def set_tv(time)
    ...
    "I just set the show #{$episode[0].series_name} in your calendar for next #{$episode[0].airs_day_of_week} at #{$episode[0].airs_time}"
